...
export default function ExpiredComponent() {
  ...
  const fetchCompletedRequests = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/getExpiredDocs`, {
      ...
      if(error?.response?.data?.error){
        toast.error(error?.response?.data?.error,{containerId:'expired'})
      }else{
        toast.error("Something went wrong please try again",{containerId:"expired"})
      }
...
toast.error("Unable to download at the moment",{containerId:"expired"})
  }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this document?")) {
      try {
       ...
       toast.success("Document deleted sucessfully",{containerId:"expired"})
        fetchCompletedRequests();
      } catch (err) {
        if(err?.response?.data?.error){
          toast.error(err?.response?.data?.error,{containerId:"expired"})
        }else{
          toast.error("Something went wrong please try again",{containerId:"expired"})
        }
  ...
  return (
    <>
    <ToastContainer containerId={"expired"}/>
    <div className="py-[8px] px-[16px] bg-white overflow-x-auto rounded-[10px] cursor-pointer min-h-[430px]">
      ...
      {paginatedRequests?.map((request) => (
        ...
        >
          <div className="font-bold text-[10px] lg:text-[16px]">{request.title}</div>

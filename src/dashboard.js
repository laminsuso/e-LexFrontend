...
export default function DashboardPage() {
  ...
  const recentSignatureRequests = useCallback(async () => {
    try {
      setLoading((p) => ({ ...p, needsignLoading: true }));
      ...
      const transformed = (response.data.documents || []).map((doc) => ({
        id: doc._id,
        title: doc.title,
        fileName: doc.file?.split("/").pop(),
        filePath: doc.file,
      ...
      setLoading((p) => ({ ...p, needsignLoading: false }));
    }
  }, []);

  const sentSignatureRequests = useCallback(async () => {
    try {
      setLoading((p) => ({ ...p, outforsignature: true }));
      ...
      const transformed = (response.data.docs || []).map((doc) => ({
        id: doc._id,
        title: doc.title,
        fileName: doc.file?.split("/").pop(),
        filePath: doc.file,
      ...
      setLoading((p) => ({ ...p, outforsignature: false }));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    sentSignatureRequests();
    getDrafts();
  }, [recentSignatureRequests, sentSignatureRequests, getDrafts]);
      ...
      <div className="flex flex-col gap-[20px] bg-transparent">
        ...
        <div className="grid lg:grid-cols-2 grid-cols-1 gap-[20px] bg-transparent">
          ...
          >
            ...
            <div className="flex flex-col gap-[6px] text-[18px] font-bold">
              Request signatures
              <div className="text-[12px] font-normal">
                Use this option to request signatures from others and yourself together.
              </div>
            </div>
          </Link>
        </div>
        ...
        <div className="grid lg:grid-cols-2 grid-cols-1 gap-[20px] bg-transparent">
          ...
          >
            ...
            <div className="flex flex-col gap-[6px] text-[18px] font-bold text-white">
              Need Your Signature
              <div className="text-[12px] font-normal text-white">{requests.length}</div>
            </div>
          </Link>
          ...
          >
            ...
            <div className="flex flex-col gap-[6px] text-[18px] font-bold text-white">
              Out for Signatures
              <div className="text-[12px] font-normal text-white">{sentRequests.length}</div>
            </div>
          </Link>
        </div>
        ...
        <div className="grid lg:grid-cols-2 grid-cols-1 gap-[20px] bg-transparent">
          <SignatureRequests
            ...
            refreshNeedSign={recentSignatureRequests} // ✅ for refresh after delegate
          />
          <SentForSignature loading={loading} requests={sentRequests} setRequests={setSentRequests} />
        </div>

// // e-LexFrontend/src/edittemplate.js
...
//       const res = await axios.get(`${BASE_URL}/getSpecificDoc/${documentId}`, {
//         headers: { authorization: `Bearer ${token}` },
//       });
...
// e-LexFrontend/src/usetemplate.js
import React, { useState, useRef, useEffect } from "react";
import { useParams } from "react-router-dom";
import { Document, Page } from "react-pdf";
import { v4 as uuidv4 } from "uuid";
...
const asEmail = (item) => (typeof item === "string" ? item : item?.email || "");

const UseTemplate = () => {
  const { documentId } = useParams();
  const id = documentId; // ✅ always use this id
  ...
  // ✅ Proper resize handling (your original effect didn’t attach a listener)
  const [containerDimensions, setContainerDimensions] = useState({
    width: 800,
    height: 0,
  });
  ...
  // Prevent scroll while dragging an element
  useEffect(() => {
    if (draggedElement) {
      document.body.style.overflow = "hidden";
      document.body.style.touchAction = "none";
  ...
  const handleToolTouchEnd = (e) => {
    ...
    if (
      ...
      if (draggedToolType === FIELD_TYPES.DROPDOWN || draggedToolType === FIELD_TYPES.RADIO) {
        setCurrentTool(draggedToolType);
        setShowOptionsModal(true);
        setDropPosition({ x, y });
      } else {
  ...
  const handleMouseMove = (e) => {
    if (!draggedElement) return;
    const { x: cursorX, y: cursorY } = getContainerPosition(e.clientX, e.clientY);

    setSignatureElements((prev) =>
      prev.map((el) =>
        el.id === draggedElement
          ? { ...el, x: cursorX - positionOffset.x, y: cursorY - positionOffset.y }
          : el
      )
    );
  };
  ...
  const handleElementTouchMove = (e) => {
    ...
    e.stopPropagation();
    const touch = e.touches[0];
    const { x: cursorX, y: cursorY } = getContainerPosition(touch.clientX, touch.clientY);
  ...
  const fetchContactBook = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/fetchContactBooks`, authConfig());
      setstaticEmails(res.data.contactBooks || []);
      setDummyEmails(res.data.contactBooks || []);
    } catch (e) {
      toast.error(
  ...
  const fetchTemplate = async () => {
    try {
      // ✅ FIXED: correct id variable + include auth header
      const res = await axios.get(`${BASE_URL}/getSpecificDoc/${id}`, authConfig());
      ...
      const transformedRecipients = (res.data.doc.elements || []).map((val) => ({
        email: val.recipientEmail,
        role: val.recipientRole,
      }));

      const transformedRoles = transformedRecipients.map((val) => ({
        roleName: val.role,
        roleValue: val.role,
      }));
  ...
  const handleOptionsSubmit = () => {
    const options = {
      options: currentTool === FIELD_TYPES.DROPDOWN ? dropdownOptions : radioOptions,
    };
    createPlaceholder(currentTool, dropPosition.x, dropPosition.y, options);
    setShowOptionsModal(false);
    setDropdownOptions("");
  ...
  const handleAddContact = async () => {
    try {
      ...
      setRecipients((prev) => {
        ...
        if (findIndex >= 0) {
          old[findIndex] = { ...old[findIndex], email: newContact.email };
        } else {
          old.push({ email: newContact.email, role: currentRole });
        }
      ...
      setSignatureElements((prev) => {
        let old = [...prev];
        const findIndex = old.findIndex(
          (u) => u.recipientRole === currentRole && (u.recipientEmail || "").length === 0
        );

        if (findIndex >= 0) {
          old[findIndex] = { ...old[findIndex], recipientEmail: newContact.email };
        }
  ...
  const handleSendDocument = async () => {
    try {
      ...
      const isEmpty = recipients.find((u) => (u.email || "").length === 0);
      if (isEmpty) {
        toast.error("Please assign email to all roles", {
          containerId: "editTemplate",
        });
      ...
      // ✅ FIXED: include auth header
      const res = await axios.post(`${BASE_URL}/createSignTemplate`, newData, headers);
      ...
      await axios.post(
        `${BASE_URL}/sendSignRequest`,
        { documentId: res.data.doc._id, recipients },
        headers
      );
  ...
  const createTemplateView = async () => {
    ...
    try {
      setCurrentTemplate((prev) => ({ ...prev, elements: signatureElements }));
      ...
      let signers = signatureElements.map((val) => ({
        email: val.recipientEmail,
        role: val.recipientRole,
      }));
  ...
  const renderFieldPreview = (element) => {
    const recipientIndex = recipients.findIndex((r) => r.email === element.recipientEmail);
    ...
    return (
      ...
      >
        ...
        {element.recipientEmail && (
          <div className="text-xs mt-1">
            <div>{element.recipientEmail}</div>
            {element.recipientRole && <div>({element.recipientRole})</div>}
          </div>
        )}
  ...
  const renderToolItem = (type) => {
    ...
    return (
      ...
      >
        ...
        {isMobile && (
          <span className="text-xs text-gray-500 block">Tap and drag to PDF</span>
        )}
      </div>
    );
  };
      ...
      <div className="admin-content h-full">
        ...
        >
          <div className="flex-1 p-4 overflow-auto relative">
            ...
            {signatureElements
              ?.filter(
                (element) => element.pageNumber === pageNumber || !element.pageNumber
              )
              ?.map((element) => renderFieldPreview(element))}
          </div>
          ...
          <div className="lg:w-80 w-full bg-white p-4 shadow-lg overflow-y-auto lg:max-h-screen max-h-[300px]">
            <div className="mb-6">
              ...
              {recipients.length > 0 ? (
                ...
                <p className="text-sm text-gray-500 mb-4">No recipients added yet</p>
              )}

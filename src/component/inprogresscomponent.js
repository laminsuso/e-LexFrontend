...
export default function InprogressComponent() {
  ...
  const fetchInProgressRequests = async () => {
    try {
      ...
      const response = await api.get("/getInProgressDocs");

      const transformedData = (response.data.documents || [])
        // ✅ defensive: ensure drafts do not show up
        .filter((doc) => doc?.draft !== true)
        // ✅ ensure "in progress" meaning: not completed/declined
        .filter((doc) => {
          const st = (doc.status || "").toLowerCase();
          if (st === "completed") return false;
          if (st === "declined") return false;
          // Keep "pending" and "sent" as in-progress
          return st === "pending" || st === "sent" || st === "in progress" || st === "inprogress";
        })
        .map((doc) => ({
          id: doc._id,
          title: doc.title,
  ...
  /**
   * ✅ Copy in-progress → Draft copy
   * - creates draft:true, status:"draft"
   * - DOES NOT navigate to request signatures
   * - refreshes list
   */
  const handleCopyToDraft = async (docId) => {
    try {
      await api.post("/duplicateEnvelope", {
        sourceDocId: docId,
        asDraft: true,       // ✅ important
        resetElements: true, // ✅ clear field values
      });

      toast.success("Copied to Drafts", { containerId: "inprogress" });
      fetchInProgressRequests(); // refresh
    } catch (e) {
      toast.error(e?.response?.data?.error || "Unable to copy to drafts", {
        containerId: "inprogress",
      });
    }
  };

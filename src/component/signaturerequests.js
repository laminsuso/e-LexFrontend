...
export default function SignatureRequests({
  ...
  try {
    ...
    const emailSent = resp?.data?.emailSent;     // ✅ backend should return this
    const signUrl = resp?.data?.signUrl;         // ✅ backend should return this when email fails
    ...
    // ✅ Success messaging depends on whether email actually sent
    if (emailSent === false && signUrl) {
      // Try auto-copy the link for you
      try {
        await navigator.clipboard.writeText(signUrl);
        toast.warning(
          msg || "Delegated, but email could not be sent. Link copied to clipboard.",
          { containerId: "signatureRequests" }
        );
      } catch {
        toast.warning(
          (msg || "Delegated, but email could not be sent.") + " Copy link manually: " + signUrl,
          { containerId: "signatureRequests" }
        );
      }
      return;
    ...
    // Default success (emailSent true or backend doesn't include that field yet)
    toast.success(
      msg || "Delegated successfully. The new signer has been invited.",
      { containerId: "signatureRequests" }
    );
  } catch (e) {
    toast.error(e?.response?.data?.error || "Unable to delegate", {

...
export default function Needsign() {
  ...
  const [requests, setRequests] = useState([]);
  const [currentEmail, setCurrentEmail] = useState(""); // from API
  const [showSignersPopup, setShowSignersPopup] = useState(false);
  const [currentSigners, setCurrentSigners] = useState([]);
  const [loading, setLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  ...
  const fetchNeedSignRequests = async () => {
    try {
      ...
      // ✅ Uses api instance (auto-attaches Authorization header)
      const response = await api.get("/getNeedSignDocs");
      ...
      const transformedData = (response.data?.documents || []).map((doc) => ({
        ...
        createdAt: doc.createdAt,
        updatedAt: doc.updatedAt,
        expiryDate: doc.createdAt ? new Date(doc.createdAt).toLocaleDateString() : "",
        status: doc.status,
      }));
      ...
      toast.error(msg, { containerId: "Needsign" });
      console.error("Error fetching need sign requests:", error);
    } finally {
      setLoading(false);
    }
  ...
  // If filters change, reset pagination to page 1 (quality-of-life)
  useEffect(() => {
    setCurrentPage(1);
  ...
  const submitDelegate = async () => {
    try {
      ...
      // ✅ Uses api instance
      await api.patch("/delegateSigner", {
        documentId: delegateDoc.id,
        fromEmail: currentEmail,
        toEmail: delegateEmail.trim(),
        toName: delegateName.trim(),

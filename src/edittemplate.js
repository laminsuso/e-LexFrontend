...
const EditTemplate = () => {
  ...
  const params = useParams();
  const id = params.documentId; // ✅ always use this id
  ...
  // ✅ FIXED: touch tool drag/drop (no email arg, and correct createPlaceholder call)
  const handleToolTouchStart = (type, options = {}) => (e) => {
    e.preventDefault();
    const touch = e.touches[0];

    setTouchDraggedElement({ type, options });
    setTouchStartPos({
      x: touch.clientX,
      y: touch.clientY,
    });
  ...
  const handleToolTouchEnd = (e) => {
    ...
    if (x >= 0 && x <= rect.width && y >= 0 && y <= rect.height) {
      const { type, options } = touchDraggedElement;
      // ✅ correct signature: (type, x, y, options)
      createPlaceholder(type, x, y, options);
    }
  ...
  const handleSaveTemplate = async () => {
    try {
      ...
      setTimeout(() => navigate("/admin/template/manage"), 100);
    } catch (error) {
      toast.error(
    ...
    <>
      ...
      <div className="admin-content">
        ...
        >
          ...
          {/* Assign modal (kept, but fixed email rendering) */}
          {showAssignmentModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white p-6 rounded-lg w-96">
                <h3 className="text-xl font-bold mb-4">Assign Field</h3>
          ...
          {/* Add recipient modal (fixed to support string or object) */}
          {showAddRecipientModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white p-6 rounded-lg w-96">
                <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (recipientExists) {
                        toast.error("This recipient has already been selected.", {
                          containerId: "editTemplate",
                        });
                        return;
                      }
                      ...
                      setRecipients([
                        ...recipients,
                        { email: selectedRecipient, role: "Signer" },
                      ]);
